...
import { Target, Shield, Zap, Heart, Stethoscope, CheckCircle2, ArrowRight } from 'lucide-react';
...
const values: Value[] = [
  ...
  {
    title: 'Simplicity',
    description: 'No expensive hardware. No specialist training. iPhone in your pocket. Full-body scan in about 60 seconds.',
    icon: Zap,
  },
  {
    title: 'Impact',
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-14">
            {problemStats.map((stat, i) => (
              <FadeUp key={stat.value} delay={i * 0.1}>
                <div className="bg-cream rounded-3xl p-8 border border-gray-200 text-center">
                  <div className="text-5xl md:text-6xl font-extrabold text-coral mb-3 tracking-tight">{stat.value}</div>
                  <p className="text-navy text-sm font-semibold mb-2 leading-snug">{stat.label}</p>
                  <p className="text-gray-400 text-xs leading-relaxed">{stat.emphasis}</p>
                </div>
              </FadeUp>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
            <FadeUp>
              ...
              <p className="text-gray-500 leading-relaxed">
                ...
                Total Body Photography should be a standard part of every physician&apos;s practice —
                not a privilege of subspecialty clinics with expensive equipment. Because the technology
                to save lives should never require a fellowship — or a six-figure hardware budget — to deploy.
              </p>
            </FadeUp>
      ...
      <Section background="white">
        <Container>
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            <FadeUp>
              <SectionEyebrow>Where We&apos;re Going</SectionEyebrow>
              <SectionHeadline className="text-navy mt-3 mb-5">
            ...
            <FadeUp delay={0.15}>
              <div className="space-y-4">
                {[
                  { label: 'FDA De Novo Clearance', status: 'In Progress', done: false, sub: 'Regulatory pathway active. Strategy established with specialist counsel.' },
                  { label: 'EHR Integration', status: 'Planned', done: false, sub: 'Direct export to Epic, Cerner, and major EHR platforms for seamless workflow.' },
                  { label: 'National Physician Deployment', status: 'Planned', done: false, sub: 'Expanding from early access cohort to 10,000+ physicians across the U.S.' },
                  { label: 'Research Institution Partnership', status: 'Active', done: true, sub: 'Currently being tested in collaboration with a top medical research institution.' },
                ].map(({ label, status, done, sub }) => (
                  <div key={label} className={`flex gap-4 items-start rounded-2xl p-5 border ${done ? 'border-coral/20 bg-coral/5' : 'border-gray-200 bg-cream'}`}>
                    <div className={`w-8 h-8 rounded-full shrink-0 flex items-center justify-center mt-0.5 ${done ? 'bg-coral' : 'bg-gray-200'}`}>
                      <CheckCircle2 size={14} className={done ? 'text-white' : 'text-gray-400'} aria-hidden="true" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-navy text-sm">{label}</span>
                        <span className={`text-2xs font-semibold px-2 py-0.5 rounded-full ${done ? 'bg-coral/15 text-coral' : 'bg-navy/10 text-navy/60'}`}>{status}</span>
                      </div>
